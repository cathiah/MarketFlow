...
import { getLayoutData } from "./layout.server";
import type { UserRole } from "~/types/profile";
import type { Route } from "./+types/_layout";

export async function loader({ request }: Route.LoaderArgs) {
  return await getLayoutData(request);
}

export default function HomeLayout({ loaderData: { user, unreadCount } }: Route.ComponentProps) {
  const role: UserRole = user?.role ?? "buyer";
  const isAuthorized = useMemo(
    () => (["admin", "seller"] as UserRole[]).includes(role),
    [role]
  );
  ...
  return (
    <div data-theme="dracula" className="min-h-screen bg-base-300 text-base-content font-sans">
      <Navbar user={user} isAuthorized={isAuthorized} unreadCount={unreadCount} />
